...
function set_patient_values(frm) {
	...
	// 🟢 RETURN PROMISE (FIX)
	return frappe.db.get_value('Patient', frm.doc.patient, [
		'patient_name',
		'mobile',
		'sex',
		'sr_patient_id',
...
function trigger_slot_dialog(frm) {
	...
	if (frm.doc.appointment_time) return;
	...
	frappe.call({
		...
		let d = new frappe.ui.Dialog({
			...
			primary_action() {
				...
				frm.set_value("appointment_time", d.selected_slot).then(() => {
					frm.save();  // auto save after selection
				});
		...
		// ✅ FIX: prevent multiple event bindings
		d.$wrapper
			.off('click', '.slot-btn')
